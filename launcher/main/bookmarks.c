...
// 署名 tab 的事件处理
static void credits_event_handler(gui_event_t event, tab_t *tab)
{
    if (event == TAB_INIT)
    {
        gui_resize_list(tab, 5);
        sprintf(tab->listbox.items[0].text, "Xueersi Retro-Go");
        sprintf(tab->listbox.items[1].text, "Ported by 懒狗");
        sprintf(tab->listbox.items[2].text, "QQ群: 724167029");
        sprintf(tab->listbox.items[3].text, "Based on Retro-Go");
        sprintf(tab->listbox.items[4].text, "github.com/zccxcsfsdfdf");
...
void bookmarks_init(void)
{
    ...
    // 署名 tab
    gui_add_tab("credits", "Ported by 懒狗", NULL, credits_event_handler);
}